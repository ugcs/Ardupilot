...
//  status.flags.vert_pos = !hgtTimeout && filterHealthy;            // vertical position estimate valid
    status.flags.vert_pos = !hgtTimeout && filterHealthy && !hgtNotAccurate;
...
bool NavEKF::calcGpsGoodToAlign(void)
{
    ...
    // fail if gps is not reach it's best status to be set correctly
    bool fixStatusFail =  _ahrs->get_gps().status() < _ahrs->get_gps().highest_supported_status();
    if (fixStatusFail) {
        hal.util->snprintf(prearm_fail_string, sizeof(prearm_fail_string),
                           "GPS status %u (needs %u)", _ahrs->get_gps().status(), _ahrs->get_gps().highest_supported_status());
    }
    
    // record time of fail if failing
    if (gpsVelFail || numSatsFail || hAccFail || yawFail || fixStatusFail) {
