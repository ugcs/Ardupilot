...
/// NORTH mode (SiRF, MediaTek and ublox) to emit the correct message
...
#ifndef __AP_GPS_NORTH_H__
#define __AP_GPS_NORTH_H__

#include "AP_GPS.h"

/// NORTH parser
///
class AP_GPS_NORTH : public AP_GPS_Backend
{
public:
	AP_GPS_NORTH(AP_GPS &_gps, AP_GPS::GPS_State &_state, AP_HAL::UARTDriver *_port);

    /// Checks the serial receive buffer for characters,
    /// attempts to parse NORTH data and updates internal state
    /// accordingly.
    bool        read();

	static bool _detect(struct NORTH_detect_state &state, uint8_t data);
...
private:
    ...
    ///
    /// @param	c		The next character in the NORTH input stream
    ...
    /// Parses the current term as a NORTH-style decimal number with
    ...
    /// Parses the current term as a NORTH-style degrees + minutes
    ...
    /// return true if we have a new set of NORTH messages
    bool _have_new_message(void);

    uint8_t _parity;                                                    ///< NORTH message checksum accumulator
	...
	uint8_t _gps_status;											/// < 4 for RTK-fixed and 5 for RTK-float
    ...
    //@{
    static const prog_char _gprmc_string1[];
    static const prog_char _gpgga_string1[];
    static const prog_char _gpvtg_string1[];
	
    static const prog_char _gprmc_string2[];
    static const prog_char _gpgga_string2[];
    static const prog_char _gpvtg_string2[];

	static const prog_char _gprmc_string3[];
    static const prog_char _gpgga_string3[];
    static const prog_char _gpvtg_string3[];
    //@}

    static const prog_char _initialisation_blob[];
};

#endif // __AP_GPS_NORTH_H__
