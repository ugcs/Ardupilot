...
void SITL_State::_update_gps_ubx(const struct gps_data *d)
{
    ...
    pos.altitude_ellipsoid = d->altitude*1000.0f + 500. * cosf(period_timer*3);
    pos.altitude_msl = d->altitude*1000.0f  + 1000. * cosf(period_timer*3);
...
//    pvt.flags = 0b10000011; // carrsoln=fixed, psm = na, diffsoln and fixok
    pvt.flags = 0b00000011; // carrsoln=fixed, psm = na, diffsoln and fixok

    pvt.flags2 =0;
    pvt.num_sv = d->have_lock?_sitl->gps_numsats:3; 

    pvt.lon = pos.longitude;// = d->longitude * 1.0e7 + 100. * _sitl-> gps2_noise * sinf(0.9 * period_timer);
    pvt.lat = pos.latitude;//  = d->latitude * 1.0e7 +  100. * _sitl-> gps2_noise * sinf(1.2 * period_timer);
    pvt.height = pos.altitude_ellipsoid;//= d->altitude*1000.0f + 500. * cosf(period_timer);
    pvt.h_msl = pos.altitude_msl;// = d->altitude*1000.0f  + 500. * cosf(period_timer);
...
void SITL_State::_update_gps_nova(const struct gps_data *d)
{
    ...
    if (!d->have_lock) {
        bestvel.vertspd += ( 100. - ((unsigned)random()%200) ) / 500.;
    }
...
bool SITL_State::_update_gps(struct  gps_data d, SITL::SITL::GPSType type, gps_data* data_list, const Vector3f& glitch_offsets, bool have_lock, int fd, uint8_t *gps_index_ptr) {
    ...
    if (_sitl->gps_drift_alt > 0 && current_gps_fd == gps2_state.gps_fd) {
        // slow altitude drift
